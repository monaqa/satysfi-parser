...
use crate::{Pair, Rule, SatysfiParser};
...
pub struct ParsedAst {
    ...
    pub inner: Option<Vec<ParsedAst>>,
}

impl ParsedAst {
    pub fn check_parsed(&self) -> std::result::Result<Pair, AstParseError> {
        let text = &self.text;
        let rule = self.rule;
...
pub struct AstBranchSequence(Vec<(usize, Rule)>);
